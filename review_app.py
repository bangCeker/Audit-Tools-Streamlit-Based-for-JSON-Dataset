...
def open_job_cb(job_no: int, all_ids: list):
    ...
    reviewed = st.session_state.reviewed_ids
...
# =====================
# Review State (persist reviewed/changed per job_size)
# =====================
def _review_state_path(dataset_path: str, job_size: int) -> str:
    """
    Simpan state terpisah per dataset + job_size
    contoh: train.review_state.job100.json
    """
    base_dir = os.path.dirname(dataset_path) or "."
    base_name = os.path.splitext(os.path.basename(dataset_path))[0]
    return os.path.join(base_dir, f"{base_name}.review_state.job{int(job_size)}.json")


def load_review_state(path: str):
    """
    Return:
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            d = json.load(f) or {}
        reviewed = set(str(x) for x in (d.get("reviewed_ids") or []))
        changed  = set(str(x) for x in (d.get("changed_ids") or []))
        notes    = d.get("notes_map") or {}
        # pastikan string
        notes = {str(k): str(v) for k, v in notes.items()}
        return reviewed, changed, notes
    except Exception:
        # kalau file corrupt/format beda
        return set(), set(), {}


def save_review_state(path: str, reviewed_ids, changed_ids, notes_map: dict):
    """
    reviewed_ids/changed_ids bisa set/list
    """
    if not path:
        return
    os.makedirs(os.path.dirname(path) or ".", exist_ok=True)
    payload = {
        "reviewed_ids": sorted(list(set(str(x) for x in reviewed_ids))),
        "changed_ids":  sorted(list(set(str(x) for x in changed_ids))),
        "notes_map":    {str(k): str(v) for k, v in (notes_map or {}).items()},
        "updated_at":   datetime.now().isoformat(timespec="seconds"),
    }
    with open(path, "w", encoding="utf-8") as f:
...
with st.sidebar:
    ...
    st.divider()
    st.header("Review Scope (Opsi B)")
    status_view = st.radio(
        "Tampilkan data",
        ["Unreviewed (default)", "Changed only", "Reviewed only", "All"],
        index=0,
        key="status_view",
    )
    hide_reviewed = st.checkbox("Hide reviewed dari hasil filter", value=True, key="hide_reviewed")
...
if st.session_state.get("dataset_path") != data_path:
    ...
    # --- orig snapshot untuk deteksi changed (PAKAI rows yg sudah diload) ---
    st.session_state.orig_map = {}
    for r in st.session_state.rows:
        rid = str(r["_id"])
        st.session_state.orig_map[rid] = {
            "text": r.get("text", "") or "",
    ...
    # --- review state persist per job_size ---
    st.session_state.review_state_path = _review_state_path(data_path, job_size)
    reviewed_set, changed_set, notes = load_review_state(st.session_state.review_state_path)
    st.session_state.reviewed_ids = reviewed_set
    st.session_state.changed_ids = changed_set
    st.session_state.notes_map = notes

    # job runtime state (jangan wipe reviewed_ids!)
...
if "reviewed_ids" not in st.session_state:
    st.session_state.reviewed_ids = set()
if "changed_ids" not in st.session_state:
    st.session_state.changed_ids = set()
if "notes_map" not in st.session_state:
    st.session_state.notes_map = {}
if "orig_map" not in st.session_state:
    st.session_state.orig_map = {}
if "review_state_path" not in st.session_state:
    st.session_state.review_state_path = _review_state_path(data_path, job_size)

# reload review_state kalau job_size berubah
if st.session_state.get("_rs_job_size") != int(job_size):
    st.session_state._rs_job_size = int(job_size)
    st.session_state.review_state_path = _review_state_path(data_path, job_size)
    reviewed_set, changed_set, notes = load_review_state(st.session_state.review_state_path)
    st.session_state.reviewed_ids = reviewed_set
    st.session_state.changed_ids = changed_set
...
# =====================
# APPLY STATUS VIEW (Opsi B)
# =====================
reviewed = st.session_state.reviewed_ids
changed  = st.session_state.changed_ids

if status_view == "Unreviewed (default)":
    base_ids = [cid for cid in item_ids if cid not in reviewed]
elif status_view == "Changed only":
    base_ids = [cid for cid in item_ids if cid in changed]
elif status_view == "Reviewed only":
    base_ids = [cid for cid in item_ids if cid in reviewed]
else:
    base_ids = list(item_ids)

# hide_reviewed tambahan (kalau user mau tetap hide walau status_view bukan unreviewed)
if hide_reviewed and status_view != "Reviewed only":
    base_ids = [cid for cid in base_ids if cid not in reviewed]

if not base_ids:
    st.info("Hasil filter kosong setelah apply review-status (semua sudah direview / tidak ada data).")
    st.stop()

# =====================
# Jobs + Active IDs + Persist load
# =====================
ensure_jobs(base_ids, job_size)

cur_dataset_sig = dataset_sig_nojob([r["_id"] for r in rows])
cur_job_sig = job_sig(base_ids, job_size)
...
# recompute setelah load (biar sesuai active_job_no terbaru)
active_ids = get_active_ids(base_ids)
...
def save_change(cur_id: str, new_text: str, new_intent: str, new_urg: str, new_events: list, note: str = "", reasons: str = ""):
    pos = id2pos[cur_id]
    r = rows[pos]
    ...
    st.session_state.history.append({
        "id": cur_id,
        "pos": pos,
        "old_text": old_text,          # <-- tambah
        "old_intent": old_intent,
        "old_urgency": old_urg,
        "old_events": old_ev,
    })

    r["text"] = new_text               # <-- commit text
    r["intent"] = new_intent
    r["urgency"] = new_urg
    r["events"] = new_ev
    ...
    append_log(log_path, {
        ...
        "id": cur_id,
        "idx": r.get("_idx"),
        "old_text": old_text, "new_text": new_text,         # <-- tambah
...
def undo_last():
    ...
    pos = last["pos"]
    rows[pos]["text"] = last.get("old_text", rows[pos].get("text",""))
    rows[pos]["intent"] = last["old_intent"]
    rows[pos]["urgency"] = last["old_urgency"]
    rows[pos]["events"] = last["old_events"]
    return True

def _is_changed_now(rid: str) -> bool:
    pos = id2pos.get(rid)
    if pos is None:
        return False
    cur = rows[pos]
    orig = st.session_state.orig_map.get(rid, {})

    cur_text = (cur.get("text", "") or "")
    cur_int = (cur.get("intent", "") or "")
    cur_urg = (cur.get("urgency", "") or "")
    cur_ev  = stable_events(cur.get("events", []))

    return (
        cur_text != (orig.get("text", "") or "") or
        cur_int  != (orig.get("intent", "") or "") or
        cur_urg  != (orig.get("urgency", "") or "") or
        cur_ev   != stable_events(orig.get("events", []))
    )

def mark_reviewed(rid: str):
    st.session_state.reviewed_ids.add(str(rid))
    # kalau berubah, masuk changed_ids
    if _is_changed_now(str(rid)):
        st.session_state.changed_ids.add(str(rid))
    # persist
    save_review_state(
        st.session_state.review_state_path,
        st.session_state.reviewed_ids,
        st.session_state.changed_ids,
        st.session_state.notes_map,
    )

def set_note(rid: str, note: str):
    rid = str(rid)
    if note:
        st.session_state.notes_map[rid] = note
    else:
        st.session_state.notes_map.pop(rid, None)
    save_review_state(
        st.session_state.review_state_path,
        st.session_state.reviewed_ids,
        st.session_state.changed_ids,
...
if work_view == "Jobs":
    ...
    for i, job in enumerate(view_jobs):
        done, tot, pct = job_progress(job, base_ids)
        ...
        if col.button(
            ...
            key=f"open_job_{job['job_no']}",
            on_click=open_job_cb,
            args=(job["job_no"], base_ids),
        ):
            pass
            persist_state_now(state_path, cur_dataset_sig, cur_job_sig)
            st.rerun()
    ...
    if st.button("▶ Resume: buka job pertama yang belum 100%", use_container_width=True):
        for job in jobs:
            ...
            if tot > 0 and done < tot:
                open_job_cb(job["job_no"], base_ids)
                persist_state_now(state_path, cur_dataset_sig, cur_job_sig)
                st.rerun()
                break
        else:
...
if st.session_state.get("_cur_loaded_id") != cur_id:
    ...
    st.session_state.edit_text = cur.get("text", "") or ""
...
with left:
    ...
    preview_text = st.session_state.get("edit_text", cur.get("text","") or "")
    st.markdown(
        f"<div class='mzone-textbox'>{highlight_text(preview_text)}</div>",
        unsafe_allow_html=True,
    )
...
with right:
    ...
    if job_no:
        ...
        if jj:
            done, tot, pct = job_progress(jj, base_ids)
            st.progress(pct)
    ...
    with st.expander("Edit Text", expanded=True):
        st.text_area(
            "Text dataset",
            key="edit_text",
            height=160,
            help="Edit teks asli dataset. Perubahan akan masuk changed kalau kamu Save / Next (kalau Next auto-save).",
        )
    ...
    if r2.button("Next ➡", use_container_width=True):
        # commit dulu dari edit_* ke rows (biar _is_changed_now bisa detect)
        reasons = (qrow.get("reasons", "") if qrow else "")
        _ = save_change(
            cur_id,
            st.session_state.edit_text,      # <-- tambah
        ...
        set_note(cur_id, st.session_state.get("edit_note", ""))
    ...
    r3, r4 = st.columns(2)
    if r3.button("Save Changes 💾", use_container_width=True):
        reasons = (qrow.get("reasons", "") if qrow else "")
        set_note(cur_id, st.session_state.edit_note)
        _ = save_change(
            cur_id,
            st.session_state.edit_intent,
            st.session_state.edit_urgency,
